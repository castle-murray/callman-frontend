...
export function AddCallTime() {
    const navigate = useNavigate()
    const { slug } = useParams()
    const [searchParams] = useSearchParams()
    const copySlug = searchParams.get('copy')
    ...
    const original = data?.call_times?.find(ct => ct.slug === copySlug)
    ...
    useEffect(() => {
        if (original) {
            setFormData({
                ...
                date: original.date || '',
                time: original.time || '',
                minimum_hours: original.minimum_hours || '',
                message: original.message || ''
            })
        } else if (event?.is_single_day && event?.start_date) {
            setFormData(prev => ({
                ...prev,
    ...
    return (
        <div>
            <h1 className="text-2xl font-bold mb-6 text-text-heading dark:text-dark-text-primary">{copySlug ? 'Copy' : 'Create New'} Call Time for {event.event_name}</h1>
            <div className="bg-card-bg p-6 rounded-lg shadow-md max-w-2xl mx-auto dark:bg-dark-card-bg dark:shadow-dark-shadow">
                <form onSubmit={handleSubmit}>
                    ...
                    >
                        {callTimeMutation.isPending ? (copySlug ? 'Copying...' : 'Creating...') : (copySlug ? 'Copy Call Time' : 'Create Call Time')}
                    </button>
                    <button 
                        type="button"
