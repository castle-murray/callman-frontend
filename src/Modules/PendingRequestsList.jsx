...
export function PendingRequestsList() {
    ...
    const { data, error, isLoading } = useQuery({
        ...
        queryFn: async () => {
            const response = await api.get('/pending-list/')
    ...
    const actionMutation = useMutation({
        mutationFn: async ({ token, action }) => {
            const response = await api.post(`/request/${token}/action/`, { action })
            return response.data
        },
        onSuccess: (_, { action }) => {
            queryClient.invalidateQueries({ queryKey: ['pendingList'] })
            queryClient.invalidateQueries({ queryKey: ['pendingRequests'] })
            addMessage(action === 'confirm' ? 'Request confirmed' : 'Request deleted', 'success')
        },
        onError: (error) => {
            addMessage(error.response?.data?.message || 'Action failed', 'error')
        }
    })
    ...
    return (
        <div className="max-w-4xl mx-auto p-6">
            <h1 className="text-2xl font-bold mb-6 text-text-heading dark:text-dark-text-primary">Pending Requests</h1>
            <div className="bg-card-bg p-6 rounded-lg shadow-md dark:bg-dark-card-bg dark:shadow-dark-shadow">
                {requests.length > 0 ? (
                        ...
                        {requests.map(request => (
                            <li key={request.id} className="flex items-center justify-between border-b py-2 dark:border-dark-border-dark">
                                ...
                                <div className="flex gap-2 ml-4 shrink-0">
                                    <button
                                        onClick={() => actionMutation.mutate({ token: request.token_short, action: 'confirm' })}
                                        disabled={actionMutation.isPending}
                                        className="bg-success text-dark-text-primary px-2 py-1 rounded hover:bg-success dark:bg-dark-success dark:hover:bg-dark-success disabled:opacity-50"
                                    >
                                        Confirm
                                    </button>
                                    <button
                                        onClick={() => actionMutation.mutate({ token: request.token_short, action: 'delete' })}
                                        disabled={actionMutation.isPending}
                    ...
                    <p className="text-text-secondary dark:text-dark-text-secondary">No pending requests found.</p>
                )}
            </div>
            <button
